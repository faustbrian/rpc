...
/**
 * Transforms Laravel validation rules into JSON Schema format.
 *
...
final class RuleTransformer
{
     ...
     *
     * @return array<string, mixed> JSON Schema properties for the field
     */
    public static function transform(string $field, array $rules): array
    {
        ...
        foreach ($rules as $rule) {
            ...
            if (str_starts_with((string) $rule, 'exclude_unless:')) {
                [$otherField, $otherValue] = explode(',', mb_substr((string) $rule, 15));
            ...
            // Exists (Database)
            throw_if(str_starts_with((string) $rule, 'exists:'), JsonSchemaException::invalidRule('exists'));

            // Extensions
            throw_if(str_starts_with((string) $rule, 'extensions:'), JsonSchemaException::invalidRule('extensions'));
            ...
            throw_if(str_starts_with((string) $rule, 'mimes:'), JsonSchemaException::invalidRule('mimes'));
            ...
            if (str_starts_with((string) $rule, 'missing_with_all:')) {
                $otherFields = explode(',', mb_substr((string) $rule, 17));
            ...
            if (str_starts_with((string) $rule, 'present_with_all:')) {
                $requiredFields = explode(',', mb_substr((string) $rule, 17));
            ...
            if (str_starts_with((string) $rule, 'prohibited_unless:')) {
                [$otherField, $value] = explode(',', mb_substr((string) $rule, 18));
            ...
            if (str_starts_with((string) $rule, 'required_unless:')) {
                [$otherField, $value] = explode(',', mb_substr((string) $rule, 16));
                $fieldSchema['allOf'][] = [
                    'if' => [
            ...
            if (str_starts_with((string) $rule, 'required_with:')) {
                $otherFields = explode(',', mb_substr((string) $rule, 14));
            ...
            if (str_starts_with((string) $rule, 'required_without:')) {
                $fieldSchema['dependencies'][$field] = [
                    'not' => array_map(
                        fn (string $f): array => ['required' => [$f]],
                        explode(',', mb_substr((string) $rule, 17)),
                    ),
                ];
            }
            ...
            // Same
            throw_if(str_starts_with((string) $rule, 'same:'), JsonSchemaException::invalidRule('same'));
            ...
            // Unique (Database)
            throw_if(str_starts_with((string) $rule, 'unique:'), JsonSchemaException::invalidRule('unique'));
